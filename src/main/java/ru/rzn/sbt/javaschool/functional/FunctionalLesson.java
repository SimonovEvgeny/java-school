...
//    map.forEach((key, value) -> sb.append(String.format("%s=%s\n", key, value)));
   ...
   */
  public long count_f() throws IOException {
    return Files
            .lines(Paths.get(THIS_FILE))
            .count();
  }
   ...
   */
  public String fWord() throws IOException {
   ...
   * @return Integer List
   * SIZE - размер листа
   * MAX_VALUE_INTEGER - максимальное значение
   */
  List<Integer> getRandomIntegerList() {
    final int SIZE = 5;
    final int MAX_VALUE_INTEGER = 20;
    return Stream.generate(() -> (int) (Math.random() * MAX_VALUE_INTEGER))
            .filter(i -> i > 0 & i < MAX_VALUE_INTEGER)
            .limit(SIZE)
            .collect(toList());
